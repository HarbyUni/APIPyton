...
class SolicitudRecoleccionSchema(BaseModel):
    id: str = Field(..., description="ID único de la solicitud de recolección")
...
class CreateSolicitudSchema(BaseModel):
    id: Optional[str] = Field(None, description="ID único de la solicitud de recolección")
    id_usuario: str = Field(..., description="ID del usuario que realiza la solicitud")
    fecha_recoleccion: Optional[datetime] = Field(None, description="Fecha solicitada para la recolección")
    direccion: str = Field(..., description="Dirección de recolección")
    codigo_qr: Optional[str] = Field(None, description="Código QR generado")
...
class SolicitudBody(BaseModel):
    data: SolicitudRecoleccionSchema

class SolicitudResponse(BaseModel):
    body: SolicitudBody

class FetchSolicitudSchema(BaseModel):
    solicitudes: List[SolicitudRecoleccionSchema] = []
    total: int = 0

class FetchSolicitudBody(BaseModel):
    data: FetchSolicitudSchema

class FetchSolicitudResponse(BaseModel):
    body: FetchSolicitudBody
